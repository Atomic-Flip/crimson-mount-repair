...
Save-slot resolution:
  - If you pass an explicit path, that path is used.
  - If exactly one save.save is detected, it is used automatically.
  - If multiple are detected and stdin is interactive, the tool prompts you
    to choose one.
...
def _slot_label(slot_dir_name: str) -> str:
    """Turn a slot directory name like 'slot100' into a human label.

    Crimson Desert uses:
      - slot0..slot2    → autosaves
      - slot100..slot108 → manual saves (in-game slots 1..9)
    """
    if not slot_dir_name.startswith("slot"):
        return slot_dir_name
    try:
        n = int(slot_dir_name[4:])
    except ValueError:
        return slot_dir_name
    if 0 <= n <= 2:
        return f"slot{n} (auto {n + 1})"
    if 100 <= n <= 108:
        return f"slot{n} (manual {n - 99})"
    return slot_dir_name


def _char_id(save_path: Path) -> str:
    """Extract the numeric character-id directory from a save path, or '?'.

    Path shape: .../save/<char_id>/slotNNN/save.save
    """
    try:
        return save_path.parent.parent.name
    except Exception:
        return "?"


def _format_save_row(index: int, path: Path) -> str:
    slot = _slot_label(path.parent.name)
    char = _char_id(path)
    try:
        st = path.stat()
        mtime = datetime.fromtimestamp(st.st_mtime).strftime("%Y-%m-%d %H:%M")
        size_kb = st.st_size / 1024
        meta = f"modified {mtime}   ({size_kb:,.0f} KB)"
    except OSError:
        meta = "(metadata unavailable)"
    return f"  [{index:2d}] char {char}   {slot:<22}   {meta}"


def _print_no_saves_hint() -> None:
...
def _print_paths_table(paths: list[Path]) -> None:
    if not paths:
        _print_no_saves_hint()
        return
    # Sort by most recently modified first — usually the one the player cares about.
    paths_sorted = sorted(
        paths,
        key=lambda p: p.stat().st_mtime if p.exists() else 0,
        reverse=True,
    )
    print(f"Found {len(paths_sorted)} save file(s):")
    print()
    print("  Crimson Desert keeps up to 12 save slots per character:")
    print("     3 autosaves (slot0..slot2), 9 manual saves (slot100..slot108).")
    print()
    for i, p in enumerate(paths_sorted, 1):
        print(_format_save_row(i, p))
...
def _prompt_for_save(paths: list[Path]) -> Path | None:
    """Interactive picker. Sorts paths most-recent-first, prompts user.

    Returns the chosen Path, or None if the user aborted.
    """
    paths_sorted = sorted(
        paths,
        key=lambda p: p.stat().st_mtime if p.exists() else 0,
    ...
    print(f"Found {len(paths_sorted)} save file(s). Which one would you like to use?")
    print()
    print("  Crimson Desert keeps up to 12 save slots per character:")
    print("     3 autosaves (slot0..slot2), 9 manual saves (slot100..slot108).")
    print()
    ...
    if not sys.stdin.isatty():
        print("error: multiple saves detected but stdin is not interactive.",
              file=sys.stderr)
        print("       Pass the path explicitly on the command line.", file=sys.stderr)
        return None

    while True:
        try:
            resp = input(f"Select [1-{len(paths_sorted)}, or q to quit]: ").strip().lower()
        ...
        try:
            ...
            continue
        if 0 <= idx < len(paths_sorted):
            chosen = paths_sorted[idx]
            print(f"Selected: {chosen}")
            return chosen
        print(f"  Please enter a number between 1 and {len(paths_sorted)}.")


def _resolve_save_path(user_path: str | None) -> Path | None:
    """Figure out which save file to operate on.

    Precedence:
      1. Explicit path passed on the command line.
      2. Single auto-detected save.
      3. Interactive prompt if multiple are detected and stdin is a TTY.
    Returns None if unable to resolve (error already printed).
    """
    if user_path:
...
def cmd_repair(args: argparse.Namespace) -> int:
    ...
    if not schema_v.is_supported:
        print("error: repaired save schema regression. Refusing to overwrite.",
              file=sys.stderr)
        return 1

    # Pass 1 of repair_save already rewrites every real self-referential PO
    # using the original blob as ground truth, so a byte-scan here would falsely
    # flag data bytes that happen to match the sentinel pattern. Trust that.
    del parc_v
    gc.collect()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_scan.add_argument("path", nargs="?",
                        help="path to save.save (auto-detect / prompt if omitted)")
